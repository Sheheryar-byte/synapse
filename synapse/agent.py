...
from synapse.tool import ToolFunction
...
class Agent:
    ...
    async def _handle_tool_calls(
        ...
        # Build a lookup map from the agent's own declared tools only.
        # This prevents global registry bleed: only tools listed in tools=[...]
        # on this specific agent are reachable during execution.
        agent_tools: dict[str, ToolFunction] = {
        ...
        for tc in tool_calls:
            ...
            tool_fn = agent_tools.get(fn_name)
            if tool_fn is None:
                result = f"Error: tool '{fn_name}' not found in this agent's tool list."
            else:
                try:
                    raw = tool_fn(**fn_args)
    ...
    def _build_tools_payload(self) -> list[dict]:
        """Collect tools from the agent's explicit tools=[...] config only.

        The global @tool registry is intentionally NOT used here.
        Injecting the global registry into every agent call causes bleed:
        agents without tools=[...] would receive schemas they never declared,
        causing capable models to attempt (and fail) spurious tool calls.
        """
        return [t.schema for t in self._config.tools]
